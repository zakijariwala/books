"""Generate a Pandoc reference.docx set up for a 6x9 paperback interior.

Pandoc's stock reference document is US Letter with Word's default margins.
Handing that to a print-on-demand service (KDP, IngramSpark) produces a rejected
or badly-laid-out interior. This script patches page size, margins, and
mirrored-margin behaviour so `make docx` emits something that matches the trim
size the figures were rendered for.
...
every figure (and updating normalize_image.py / figstyle.py). Keep them in step.
...
mere compliance. Adjust the constants below for a different trim.
"""
import re
import shutil
import subprocess
