...
#
# Install it once per clone:  make hooks   (or copy/symlink to .git/hooks/pre-commit)
#
# Both checks block the commit on failure. The build tests are deliberately
# excluded: they need Pandoc and a built book, which is too slow to run per
# commit. Run those with `make test-build` before publishing.
set -uo pipefail

# Find a Python. Prefer a local venv, fall back to whatever is on PATH.
