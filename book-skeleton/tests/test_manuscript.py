...
owns prose. Everything here has a print or build consequence, so it belongs in
the fast test set that gates every commit.

Tune the constants at the top for your book: the minimum chapter count and the
code-wrap width in particular. The rest encode the same print constraints the
render scripts do, and should move only if the trim size moves.
"""
import re
...
# Raise this as the book fills out; it stops an empty manuscript passing green.
MIN_CHAPTERS = 1
# Monospace overflows a 4.5in text block past roughly this many characters.
MAX_CODE_LINE = 60
# Wider tables break at 6x9 and in EPUB reflow.
MAX_TABLE_COLUMNS = 3
...
# Drafting scaffolding looks like *[FIGURE 4.1: description]*. Match that shape
# only: a case-insensitive \[FIGURE would also match every real caption.
...
def test_headings_stop_at_h3(path):
    # Deeper nesting reads badly in EPUB reflow and in a short book.
...
def test_tables_have_at_most_three_columns(path):
    for i, line in enumerate(read(path).splitlines(), 1):
        ...
        assert columns <= MAX_TABLE_COLUMNS, f"{path.name}:{i} has {columns} columns"
    ...
    for path in ALL_FILES:
        for number, target in FIGURE_REF.findall(read(path)):
            assert number not in seen, (
                f"Figure {number} used twice: {seen.get(number)} and {target}"
            )
            seen[number] = target
...
def test_figure_numbers_match_their_chapter():
    for path in CHAPTERS:
        ...
        for number, _ in FIGURE_REF.findall(read(path)):
            assert number.split(".")[0] == chapter, (
                f"{path.name} carries Figure {number}"
            )
...
def test_code_blocks_wrap_for_the_measure(path):
    ...
    for i, line in enumerate(read(path).splitlines(), 1):
        if line.startswith("```"):
            inside = not inside
        elif inside and len(line) > MAX_CODE_LINE:
            offenders.append(i)
